...
#pragma region Windows Form Designer generated code
	...
	private: void saveDeckListPlainText() {
		String^ deckNameS = deckNameBox->Text;
		if (deckNameS == "") {
			MessageBox::Show("Please enter a name for your deck and try again");
		}
		else {
			msclr::interop::marshal_context context;
			string deckName = context.marshal_as <string>(deckNameS); //converts from system string to standard string
			ofstream file;
	...
	private: void saveDeckList() {
		...
		if (deckNameS == "") {
			...
			string deckName = context.marshal_as <string>(deckNameS); //converts from system string to standard string
			ofstream file;
			file.open("program decks/" + deckName + ".txt", ios::trunc); //clear temp file
			file.close();
			file.open("program decks/" + deckName + ".txt", ios::app);
			if (creatures.size() != 0) {
				file << "Creatures\n\n";
				for (vector<card>::iterator p = creatures.begin(); p != creatures.end(); ++p) {
					file << p->amount;
		...
		if (MessageBox::Show("Any unsaved data will be lost", "Are you sure?", MessageBoxButtons::YesNo, MessageBoxIcon::Question) == Windows::Forms::DialogResult::Yes) {
			DeckBuilder::Close();
			 ...
			 void addToDeck(int amount, String^ selectedCard, String^ typeLine, String^ manaCost) {
				 ...
				 checkIfCardLegal(selectedCard, amount, cardType); //check if the card is legal in the selected format
				 updateDeckList();
			 ...
			 void checkIfCardLegal(String^ cardName, int amount, string cardType) {
				 String^ constring = L"datasource=localhost;port=3306;username=root;password=password";
				 MySqlConnection^ conDatabase = gcnew MySqlConnection(constring);
				 MySqlDataReader^ myReader;
				 String^ legalitiesS;

				 MySqlCommand^ cmdDataBase = gcnew MySqlCommand("SELECT legalities FROM mtgcards.`scryfall-default-cards` WHERE name='" + cardName + "';", conDatabase); //get details of specific card
				 try {
					 conDatabase->Open();
					 myReader = cmdDataBase->ExecuteReader();
					 while (myReader->Read()) {
				 ...
				 if (legalities.find("\"" + format + "\":" + " \"not_legal\"") != string::npos) {
					 statusLabel->Text = "Deck is not legal as " + cardName + " is illegal in this format";
				 }
				 if (cardType.find("Basic Land") != string::npos) {
					 ...
					 if (format == "standard" || format == "modern") {
						 if (amount > 4) {
							 statusLabel->Text = "Deck is not legal as you can not have more than 4 copies of " + cardName;
						 }
					 }
					 if (format == "commander") {
						 if (amount > 1) {
							 statusLabel->Text = "Deck is not legal as you can not have more than 1 copy of " + cardName;
						 }
					 }
				 }
